...
*/
SELECT w.essn, p.pnumber, w.hours FROM department d, project p, works_on w WHERE w.pno = p.pnumber AND p.dnum = d.dnumber AND d.dname = 'Research' AND w.hours > 20  ORDER BY w.essn;
--
-- JOINING 3 TABLES ---------------------------
--
/*(12B)
...
*/
SELECT e.lname, p.pnumber, p.dnum, w.hours FROM employee e, project p, works_on w WHERE e.ssn = w.essn AND w.pno = p.pnumber AND w.hours < 10 AND p.dnum = e.dno ORDER BY e.lname;
--
-- JOINING 4 TABLES -------------------------
--
/*(13B)
...
*/
SELECT DISTINCT e.ssn, e.lname, d.dependent_name, d.relationship FROM employee e, dependent d, project p, works_on w WHERE e.ssn = w.essn AND w.pno = p.pnumber AND e.ssn = d.essn AND p.plocation = 'Houston' ORDER BY e.lname;
--
-- SELF JOIN -------------------------------------------
-- 
/*(14B)
...
*/
SELECT e.ssn, e.lname, e.dno, e2.ssn, e2.lname, e2.dno FROM employee e, employee e2 WHERE e.dno != e2.dno AND e2.ssn = e.super_ssn ORDER BY e.ssn;
--
...
*/
SELECT DISTINCT e1.lname, e2.lname FROM employee e1, employee e2, works_on w1, works_on w2 WHERE e1.ssn = w1.essn AND e2.ssn = w2.essn AND e1.ssn < e2.ssn AND w1.pno = w2.pno AND w1.hours = w2.hours;
--
/*(16B)
For every employee who has more than one dependent: Find the ssn, lname, and number of dependents. Sort the result by lname
*/
SELECT e.ssn, e.lname, count(*) FROM employee e, dependent d WHERE d.essn = e.ssn GROUP BY e.lname, e.ssn HAVING count(*) > 2 ORDER BY e.lname;
-- 
/*(17B)
...
*/
SELECT p.pnumber, p.pname, COUNT(*), SUM(w.hours) FROM project p, works_on w WHERE p.pnumber = w.pno GROUP BY p.pnumber, p.pname HAVING COUNT(*) > 2 AND SUM(w.hours) < 40 ORDER BY p.pnumber;
--
-- CORRELATED SUBQUERY --------------------------------
--
/*(18B)
...
*/
SELECT e.ssn, e.lname FROM employee e WHERE e.dno = 5 AND NOT EXISTS (SELECT * FROM project p, works_on w WHERE e.ssn = w.essn AND w.pno = p.pnumber AND w.hours < 20) ORDER BY e.lname;
--
-- DIVISION ---------------------------------------------
--
